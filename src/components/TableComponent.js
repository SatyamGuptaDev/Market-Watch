...
const TableComponents = ({ CryptoData, currencyUnit }) => {
  // array of saved coins in an array
  const [ saved, setSaved] = useState([]); 


  const saveCoin = (coin) => {

    // if the coin is already saved, remove it from the saved array
    if (saved.includes(coin)) {
      setSaved(saved.filter((savedCoin) => savedCoin !== coin));
      return;
    }
    setSaved([...saved, coin]);
  }
          ...
          <tbody>
            {CryptoData && CryptoData.length > 0 ? (
              CryptoData.map((crypto, index) => (
                ...
                >
                  <td className="px-1 py-4 uppercase flex flex-row ">
                  <button onClick={() => saveCoin(crypto)}>
                    ...
                    >
                      <g clip-path="url(#clip0_16_420)">
                        ...
                        //  if saved fill with cyan else fill with grey
                          fill={saved.includes(crypto) ? "cyan" : "grey"}
                        />
                      </g>
                      <defs>
